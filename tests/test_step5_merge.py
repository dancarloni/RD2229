...
def test_pipeline_step5_skipped_when_not_existing_structure():
    ...
    has_warning = any(
        "existing_structure" in w or "struttura esistente" in w.lower()
        for w in results.warnings
    )
    # O lo skip viene registrato nella traccia
    has_trace = any(
        "existing_structure" in t or "no_applicable_templates" in t
...
def test_pipeline_step5_skipped_when_lc_none():
    ...
    has_trace = any(
        "lc_None" in t or "no_applicable_templates" in t
        for t in results.trace
    )
    assert has_warning or has_trace, (
        f"Atteso warning/trace per lc=None; "
        f"warnings={results.warnings}, trace={results.trace}"
    )
