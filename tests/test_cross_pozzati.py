...
class TestTraveContinua2Campate:
    ...
    def _modello(self) -> ModelloTelaio:
        ...
        return ModelloTelaio(
            nome="Test1", nodi=nodi, aste=aste, piani=[], zona_sismica="non_sismico"
        )
    ...
    def test_equilibrio_nodo_B(self):
        ...
        assert squilibrio < 10.0, f"Squilibrio al nodo B: {squilibrio:.2f} kg·cm (atteso < 10)"
...
class TestPortaleUnPiano:
    ...
    def _modello(self) -> ModelloTelaio:
        ...
        sez_trave = SezioneTelaio(
            ...
            h=h_trave_sez,
            I=I_trave,
            A=b_trave * h_trave_sez,
            Wx=b_trave * h_trave_sez**2 / 6,
            E=E,
            gamma=0.0,
        )
    ...
    def test_simmetria_portale_simmetrico(self):
        ...
        assert abs(abs(M_A) - abs(M_D)) < 10.0, f"Asimmetria A-D: M_A={M_A:.0f}, M_D={M_D:.0f}"
...
class TestFattoriDistribuzione:
    def _modello_semplice(self) -> ModelloTelaio:
        ...
        return ModelloTelaio(
            nome="3Campate", nodi=nodi, aste=aste, piani=[], zona_sismica="non_sismico"
        )
    ...
    def test_somma_mu_uguale_1(self):
        ...
        for id_nodo, f_nodo in fattori.items():
            ...
            assert somma == pytest.approx(1.0, abs=1e-9), f"Nodo {id_nodo}: Σμ = {somma:.6f} ≠ 1"
...
class TestEquilibrioSway:
    ...
    def _modello_2piani(self) -> ModelloTelaio:
        ...
        return ModelloTelaio(
            nome="Telaio2Piani", nodi=nodi, aste=aste, piani=piani, zona_sismica="media"
        )
    ...
    def test_momenti_finali_non_nulli(self):
        ...
        momenti_non_nulli = sum(
            1 for (Mi, Mj) in dati.momenti_finali.values() if abs(Mi) > 0.1 or abs(Mj) > 0.1
        )
        assert momenti_non_nulli >= 4, f"Troppo pochi momenti non nulli: {momenti_non_nulli}"
