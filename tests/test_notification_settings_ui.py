...
def test_ui_save_calls_save_notification_settings(monkeypatch, tmp_path):
    ...
    ns.set_settings({'level': 'all', 'show_toasts': False, 'silent_sources': ['mod1'], 'confirm_default': 'default_no'})
    ns.save()
...
def test_center_honors_level_setting(monkeypatch):
    ...
    monkeypatch.setattr('sections_app.services.notification_settings.load_notification_settings', lambda: {'level': 'errors_only', 'show_toasts': True, 'silent_sources': [], 'confirm_default': 'ask'})
