...
def test_add_remove_row_geometry(editor, qtbot):
...
def test_edit_extra_json(editor, qtbot):
    tbl = editor.tbl_geometry
    editor._add_table_row(tbl)
    row = tbl.rowCount() - 1
    ...
    # For now, dialog is stub, so value unchanged
    assert tbl.item(row, col).text() == '{"foo": 1}'
...
def test_add_remove_row_materials(editor, qtbot):
...
def test_add_remove_row_loads(editor, qtbot):
...
def test_edit_extra_materials_with_mock(editor, qtbot, monkeypatch):
    ...
    col = tbl.columnCount() - 1
    tbl.setItem(row, col, qt_api.QtWidgets.QTableWidgetItem('{"foo": 1}'))
    # Simula dialog che modifica il JSON
    monkeypatch.setattr(
        editor,
        "_show_json_edit_dialog",
        lambda current: '{"foo": 2}',
    )
...
def test_edit_extra_loads_with_mock(editor, qtbot, monkeypatch):
    ...
    monkeypatch.setattr(editor, "_show_json_edit_dialog", lambda current: '{"a": 9}')
    tbl.setCurrentCell(row, 0)
    editor._edit_extra_for_selected(tbl)
