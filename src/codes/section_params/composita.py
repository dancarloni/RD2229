...
def calcola_sezione_composta(
    *,
    ...
    log.append(f"IPE {dati.nome}: A={A_ipe:.2f} cm², I={I_ipe:.2f} cm⁴, y_G={y_G_ipe:.2f} cm")
    log.append(
    ...
    I_comp = (
        I_ipe
        + A_ipe * (y_G_ipe - y_G_comp) ** 2
        + I_sol_rid
        + A_sol_rid * (y_G_sol - y_G_comp) ** 2
    )
    ...
    log.append(
        f"Composta: A_comp={A_comp:.3f} cm², y_G={y_G_comp:.4f} cm, " f"I_comp={I_comp:.2f} cm⁴"
    )
