...
class FlexureCheck(VerificationAction):
    ...
    def _run_slu(
        self, b: float, h: float, d: float, As: float, M_Ed: float,
        normative: dict, partials: dict,
    ) -> dict[str, Any]:
        ...
        if As <= 0 or d <= 0 or b <= 0:
            return {
                "action_id": self.action_id, "ok": False,
        ...
        partials.update({
            "f_cd": round(f_cd, 1), "f_yd": round(f_yd, 1),
            "x_cm": round(x, 2), "M_Rd_kgm": round(M_Rd, 1),
            "M_Ed_kgm": round(M_Ed, 1), "utilization": round(utilization, 3),
        })

        return {
            "action_id": self.action_id, "ok": ok,
            "messages": [
    ...
    def _run_ta(
        ...
        partials.update({
            "sigma_c": round(sigma_c, 1), "sigma_c_adm": round(sigma_c_adm, 1),
            "sigma_s": round(sigma_s, 1), "sigma_s_adm": round(sigma_s_adm, 1),
            "n": n, "W_cm3": round(W, 1), "J_cm4": round(J, 1),
        })
...
class ShearCheck(VerificationAction):
    ...
    def run(
        ...
        partials: dict[str, Any] = {
            "b_cm": b, "d_cm": d, "V_Ed_kg": round(V_Ed, 1),
            "diam_st_mm": diam_st, "n_bracci": n_bracci,
            "passo_cm": passo, "A_sw_cm2": round(A_sw, 3),
        }
    ...
    def _run_slu(
        self, b: float, d: float, A_sw: float, passo: float, V_Ed: float,
        normative: dict, partials: dict,
    ) -> dict[str, Any]:
        f_yd = _material_param(normative, "f_yd", 3904.0)
    ...
    def _run_ta(
        self, b: float, d: float, V_Ed: float,
        normative: dict, partials: dict,
    ) -> dict[str, Any]:
...
class PressFlexureCheck(VerificationAction):
    ...
    def run(
        ...
        partials: dict[str, Any] = {
            "b_cm": b, "h_cm": h, "d_cm": d,
            "N_Ed_kg": round(N_Ed, 1), "M_Ed_kgm": round(M_Ed, 1),
        }
    ...
    def _run_slu(
        self, b: float, h: float, d: float, As: float,
        N_Ed: float, M_Ed: float, normative: dict, partials: dict,
    ) -> dict[str, Any]:
        ...
        partials.update({
            "N_Rd_kg": round(N_Rd, 1), "M_Rd_kgm": round(M_Rd, 1),
            "ratio_N": round(ratio_N, 3), "ratio_M": round(ratio_M, 3),
            "utilization": round(utilization, 3),
        })
    ...
    def _run_ta(
        self, b: float, h: float, N_Ed: float, M_Ed: float,
        normative: dict, partials: dict,
    ) -> dict[str, Any]:
        sigma_c_adm = _material_param(normative, "sigma_c_adm", 60.0)
        ...
        partials.update({
            "sigma_N": round(sigma_N, 2), "sigma_M": round(sigma_M, 2),
            "sigma_max": round(sigma_max, 2), "sigma_c_adm": round(sigma_c_adm, 1),
        })
...
class TorsionCheck(VerificationAction):
    ...
    def run(
        ...
        partials = {
            "b_cm": b, "h_cm": h,
            "t_ef_cm": round(t_ef, 2), "A_k_cm2": round(A_k, 1),
            "T_Ed_kgm": round(T_Ed, 1), "T_Rd_max_kgm": round(T_Rd_max, 1),
            "utilization": round(utilization, 3),
        }
...
class SLEStressCheck(VerificationAction):
    ...
    def run(
        ...
        partials = {
            "x_n_cm": round(x_n, 2), "J_r_cm4": round(J_r, 1),
            "sigma_c": round(sigma_c, 1), "lim_c": round(lim_c, 1),
            "sigma_s": round(sigma_s, 1), "lim_s": round(lim_s, 1),
            "n_ae": round(n_ae, 2),
        }
...
class SLECrackingCheck(VerificationAction):
    ...
    def run(
        ...
        kt = 0.6
        eps_term1 = (sigma_s - kt * f_ctm / max(rho_p, 0.001) * (1 + n_ae * rho_p)) / E_s if E_s > 0 else 0.0
