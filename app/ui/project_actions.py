...
from sections_app.services.notification import notify_info, notify_error
...
def _elem_dict_to_input(e: dict) -> VerificationInput:
    ...
    try:
        return VerificationInput(
            ...
            Mx=float(e.get("Mx", e.get("M", 0.0))) if e.get("Mx", None) is not None or e.get("M", None) is not None else 0.0,
            ...
            Ty=float(e.get("Ty", e.get("T", 0.0))) if e.get("Ty", None) is not None or e.get("T", None) is not None else 0.0,
            At=float(e.get("At", 0.0)) if e.get("At", None) is not None else 0.0,
            As_sup=float(e.get("As", e.get("As_sup", 0.0))) if e.get("As", None) is not None or e.get("As_sup", None) is not None else 0.0,
            As_inf=float(e.get("As_p", e.get("As_inf", 0.0))) if e.get("As_p", None) is not None or e.get("As_inf", None) is not None else 0.0,
            d_sup=float(e.get("d", e.get("d_sup", 4.0))) if e.get("d", None) is not None or e.get("d_sup", None) is not None else 4.0,
            d_inf=float(e.get("d_p", e.get("d_inf", 4.0))) if e.get("d_p", None) is not None or e.get("d_inf", None) is not None else 4.0,
            stirrup_step=float(e.get("passo_staffe", e.get("stirrup_step", 0.0))) if e.get("passo_staffe", None) is not None or e.get("stirrup_step", None) is not None else 0.0,
            stirrup_diameter=float(e.get("stirrups_diam", e.get("stirrup_diameter", 0.0))) if e.get("stirrups_diam", None) is not None or e.get("stirrup_diameter", None) is not None else 0.0,
    ...
    try:
        ...
        for el in data.get("elements") or []:
            try:
                ...
                errors.append(str(e))
        if errors:
            notify_error("Carica progetto", f"Errore caricamento progetto: {errors}", source="verification_table")
        else:
    ...
    try:
        ...
        for el in data.get("elements") or []:
            try:
                ...
                errors.append(str(e))
        notify_info("Aggiungi lista di elementi", f"Aggiunti elementi: {new_elems}", source="verification_table")
