...
def demo_basic_matplotlib():
    ...
    # Add dimensions
    ax.annotate("", xy=(0, -3), xytext=(30, -3), arrowprops=dict(arrowstyle="<->", color="black"))
    ax.text(15, -5, "b = 30 cm", ha="center", fontsize=9)

    ax.annotate("", xy=(-3, 0), xytext=(-3, 50), arrowprops=dict(arrowstyle="<->", color="black"))
...
def demo_section_visualization():
    ...
    circle = Circle((20, 20), 15, fill=True, facecolor="lightgray", edgecolor="black", linewidth=2)
...
def demo_verification_results():
    ...
    ax.set_title("Andamento Tensioni - Verifica a Flessione", fontsize=14, fontweight="bold")
    ...
    # Neutral axis
    ax.axhline(y=30, color="green", linestyle="--", linewidth=2, label="Asse Neutro")
...
def verify_installation():
    ...
    try:
        ...
        result = subprocess.run(["pip", "show", "RD2229"], capture_output=True, text=True)
