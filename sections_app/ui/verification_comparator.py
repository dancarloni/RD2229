...
from sections_app.services.notification import notify_warning, notify_info, notify_error
import logging
...
from verification_table import VerificationTableApp, compute_ta_verification, compute_slu_verification, get_section_geometry
...
class VerificationComparatorWindow(tk.Toplevel):
    ...
    @staticmethod
    def _compute_na_endpoints(b: float, h: float, p: tuple[float,float], angle_deg: float) -> tuple[tuple[float,float], tuple[float,float]]:
        ...
        return (max(0.0, min(b, px - 10*vx)), max(0.0, min(h, py - 10*vy))), (max(0.0, min(b, px + 10*vx)), max(0.0, min(h, py + 10*vy)))

    @staticmethod
    def _clip_polygon_by_halfplane(poly: list[tuple[float,float]], p: tuple[float,float], v: tuple[float,float], keep_positive: bool = True) -> list[tuple[float,float]]:
    ...
    def _compute_shaded_polygon(b: float, h: float, p: tuple[float,float], angle_deg: float, pick_side_point: tuple[float,float] = None) -> list[tuple[float,float]]:
        ...
        poly = VerificationComparatorWindow._clip_polygon_by_halfplane(rect, p, v, keep_positive=keep_positive)
        return poly
    ...
    def on_compare(self):
        ...
        try:
            ...
            mat_props = engine.get_material_properties(inp.material_concrete or '', inp.material_steel or '', material_source='RD2229')
        ...
        def _summary(res, label):
            ...
            return f"{label}: esito={getattr(res, 'esito', '')} sigma_c_max={getattr(res, 'sigma_c_max', '')} asse_neutro_x={getattr(res,'asse_neutro_x', getattr(res,'asse_neutro', ''))} inclinazione={getattr(res,'inclinazione_asse_neutro','')}\n" + (msgs[0] + '\n' if msgs else '')
        ...
        def _get_metrics(res):
            if res is None:
                return {'sigma_c_max': 0.0, 'sigma_c_min': 0.0, 'sigma_s_max': 0.0, 'asse_x': None, 'angle': None}
            if isinstance(res, dict):
                return {'sigma_c_max': res.get('Taux_max', 0.0), 'sigma_c_min': 0.0, 'sigma_s_max': 0.0, 'asse_x': None, 'angle': None}
            return {'sigma_c_max': getattr(res, 'sigma_c_max', 0.0), 'sigma_c_min': getattr(res, 'sigma_c_min', 0.0), 'sigma_s_max': getattr(res, 'sigma_s_max', 0.0), 'asse_x': getattr(res, 'asse_neutro_x', getattr(res,'asse_neutro', None)), 'angle': getattr(res,'inclinazione_asse_neutro', None)}
        ...
        for label, res, color, metrics in methods:
            ...
            try:
                self.ax_section.text(p[0], p[1], f"{label}\n{p[0]:.1f},{ang:.1f}" if isinstance(ang, (int,float)) else label, color=color)
            except Exception:
        ...
        def _get_metrics(res):
            ...
            if isinstance(res, dict):
                return {'sigma_c_max': res.get('Taux_max', 0.0), 'sigma_s_max': 0.0, 'asse_x': None, 'angle': None}
            return {'sigma_c_max': getattr(res, 'sigma_c_max', 0.0), 'sigma_s_max': getattr(res, 'sigma_s_max', 0.0), 'asse_x': getattr(res, 'asse_neutro_x', getattr(res,'asse_neutro', None)), 'angle': getattr(res,'inclinazione_asse_neutro', None)}
        ...
        # Fill summary table
        rows = [
            ('asse_neutro_x (cm)', f"{ta_metrics['asse_x']}", f"{slu_metrics['asse_x']}", f"{bas_metrics['asse_x'] if bas_metrics['asse_x'] is not None else ''}"),
            ('inclinazione (deg)', f"{ta_metrics['angle']}", f"{slu_metrics['angle']}", f"{bas_metrics['angle'] if bas_metrics['angle'] is not None else ''}"),
            ('sigma_c_max', f"{ta_metrics['sigma_c_max']}", f"{slu_metrics['sigma_c_max']}", f"{bas_metrics['sigma_c_max']}")
        ]
        for r in rows:
    ...
    def _export(self, fmt: str):
        try:
            ...
            comp = {'TA': ta_res and {'sigma_c_max': getattr(ta_res,'sigma_c_max',None),'asse_neutro_x': getattr(ta_res,'asse_neutro_x',getattr(ta_res,'asse_neutro',None)),'angle': getattr(ta_res,'inclinazione_asse_neutro',None)},
                    'SLU': slu_res and {'sigma_c_max': getattr(slu_res,'sigma_c_max',None),'asse_neutro_x': getattr(slu_res,'asse_neutro_x',getattr(slu_res,'asse_neutro',None)),'angle': getattr(slu_res,'inclinazione_asse_neutro',None)},
                    '.bas': bas_tors}
            if fmt == 'csv':
