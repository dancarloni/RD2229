...
class VerificationComparatorWindow(tk.Toplevel):
    ...
    def _compute_na_endpoints(
        ...
        """Return two endpoints (on rectangle edges) of the infinite line passing
        through p with direction angle_deg.

        Coordinate system: origin (0,0) is top-left, x rightwards, y downwards.
        angle_deg: angle in degrees measured counterclockwise from +x axis
        (standard), consistent with Matplotlib axes.
        """
        import math
    ...
    def _clip_polygon_by_halfplane(
        ...
        """Clip convex polygon `poly` by half-plane defined by the line passing
        through p with direction v.

        Keeps points where cross(v, (pt-p)) has sign >=0 if keep_positive True,
        else <=0.
