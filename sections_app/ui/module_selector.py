...
class ModuleSelectorWindow(tk.Tk):
    ...
    Questa finestra fornisce un'interfaccia utente per selezionare e avviare
    i diversi moduli dell'applicazione RD2229. Gestisce anche la creazione
    e la gestione delle finestre dei moduli aperti.
    """
    ...
    def _create_menu(self) -> None:
        ...
        Aggiunge il menu File con opzioni per esportare backup e uscire.
        """
        ...
        file_menu.add_command(
            label="Notification Settings...", command=self._open_notification_settings
        )
    ...
    def _build_ui(self) -> None:
        ...
        Crea il container principale e inizializza la vista del selettore
        moduli con tutte le specifiche delle card.
        """
        container = tk.Frame(self)
        container.pack(fill="both", expand=True)
        # prepara specs riusando i callback esistenti
        specs = [
            ModuleCardSpec(
                "Geometry module",
                "Compute and manage section geometry\n"
            ...
            ModuleCardSpec(
                ...
                "Historical allowable-stress verifications\n(stubs and data connectors for now)",
                "Open Historical",
                self._open_historical,
            ),
            ModuleCardSpec(
                "Verification Table",
                "Rapid data entry for multiple verifications\n(tabular grid with autocomplete)",
            ...
            ModuleCardSpec(
                ...
                "Open Debug Viewer",
                self._open_debug_viewer,
            ),
            ModuleCardSpec(
                "Parametri Normativa",
                "Configura parametri TA/SLU/SLE (.jsoncode)",
                None,
                lambda: None,
                extra_buttons=[
                    ("Parametri TA", lambda: self._open_code_settings("TA")),
                    ("Parametri SLU", lambda: self._open_code_settings("SLU")),
                    ("Parametri SLE", lambda: self._open_code_settings("SLE")),
                ],
            ),
            ModuleCardSpec(
                "Sections Archive",
                "Browse and manage archived sections\n(import/export, edit via Geometry)",
                "Open Sections",
                self._open_section_manager,
            ),
            ModuleCardSpec(
                "Materials Editor",
                "Manage and import historical materials\n"
            ...
            ModuleCardSpec(
                "FRC Manager",
                "Manage FRC materials (carbon/glass fibers) and properties",
                "Open FRC Manager",
                self._open_frc_manager,
            ),
            ModuleCardSpec(
                "FRC Verification",
                "Quick verification window to test FRC contributions on simple sections",
                "Open FRC Verification",
                self._open_frc_verification,
            ),
        ]
        self.view = ModuleSelectorView(container, specs)
    ...
    def _open_verification_table(self) -> None:
        """Apre il modulo Verification Table.

        Nota: Questo modulo non è ancora implementato.
        """
        logger.debug("Opening Verification Table module")
        # TODO: Implement VerificationTableWindow
        raise NotImplementedError("Verification Table module not yet implemented")
    ...
    def _open_notification_settings(self) -> None:
        """Apre la finestra delle impostazioni notifiche.

        Tenta di aprire la finestra delle impostazioni notifiche,
        con fallback in caso di errori.
        """
        try:
            win = __import__(
    ...
    def _open_section_manager(self) -> None:
        ...
        manager.bind(
            "<Destroy>", lambda e, m=manager: setattr(self, "_section_manager_window", None)
        )
    ...
    def _on_section_edit(self, section: Section) -> None:
        ...
        # Se la finestra non è stata inizializzata dal metodo precedente,
        # prova a crearla direttamente
    ...
    def _open_material_editor(self) -> None:
        ...
        def on_material_editor_close():
            ...
            if (
                self._material_editor_window is not None
                and self._material_editor_window.winfo_exists()
            ):
                try:
        ...
        try:
            ...
            # Inoltre, se la finestra viene distrutta in altro modo,
            # assicurati di pulire il riferimento
            self._material_editor_window.bind("<Destroy>", lambda e: on_material_editor_close())
        except Exception:
            pass

    def _open_frc_manager(self) -> None:
        """Apre il modulo FRC Manager.

        Gestisce i materiali FRC (fibre di carbonio/vetro) e le loro proprietà.
        """
    ...
    def _open_frc_verification(self) -> None:
        """Apre il modulo FRC Verification.

        Finestra di verifica rapida per testare i contributi FRC su sezioni semplici.
        """
    ...
    def _export_backup(self) -> None:
        ...
        Mostra un dialogo per scegliere cosa esportare (sezioni, materiali, entrambi)
        e salva i file selezionati.
        """
        ...
        tk.Label(frame, text="Cosa vuoi esportare?", font=("TkDefaultFont", 11, "bold")).pack(
            pady=(0, 20)
        )
        ...
        tk.Button(btn_frame, text="Sezioni", width=15, command=lambda: on_choice("sezioni")).pack(
            pady=5
        )

        tk.Button(
            btn_frame, text="Materiali", width=15, command=lambda: on_choice("materiali")
        ).pack(pady=5)

        tk.Button(btn_frame, text="Entrambi", width=15, command=lambda: on_choice("entrambi")).pack(
            pady=5
        )
